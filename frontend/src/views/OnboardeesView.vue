<template>
  <div class="onboardees">
    <div class="search-container">
      ...
      <Button @click="downloadCSV" class="add-button" label="CSV" icon="pi pi-download"/>
    </div>
    ...
    <div v-for="(stateOnboardees, state) in filteredOnboardees" :key="state">
      ...
      <DataTable class="custom-table" :value="stateOnboardees" :globalFilter="globalFilter" @row-click="handleRowClick"
        tableStyle="min-width: 50rem" removableSort paginator :rows="5" :rowsPerPageOptions="[5, 10, 20, 50]">
        <Column field="name" sortable header="Name" :headerStyle="{ backgroundColor: '#033A65', color: 'white' }">
        </Column>
        ...
        <Column field="startDate" header="Start date" :headerStyle="{ backgroundColor: '#033A65', color: 'white' }">
        </Column>
      </DataTable>
    </div>
  </div>
